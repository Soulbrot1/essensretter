      ...
      (foods) async {
        ...
        return recipesResult.fold(
          ...
          (recipes) async {
            ...
            final availableFoodNames = foods.map((f) => f.name.toLowerCase()).toSet();
            ...
            for (int i = 0; i < recipes.length; i++) {
              if (recipes[i].vorhanden.length != synchronizedRecipes[i].vorhanden.length ||
                  recipes[i].ueberpruefen.length != synchronizedRecipes[i].ueberpruefen.length) {
                needsUpdate = true;
                break;
              }
            }
